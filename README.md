...
| 진입점 | 내용 |
| --- | --- |
| `index.html` → `compose.tsx` | **작성 도우미(현행).** 네 가지 도구로 내용을 만들고 한글(HWP)에 복사해 붙여넣는 방식 |
| `classic.html` → `classic.tsx` | 이전 6단계 버전. A4 인쇄 출력까지 하던 앱으로, 저장해 둔 작업을 이어 쓸 수 있게 당분간 유지 |

두 페이지는 `data/`(성취기준·성취수준)와 `services/`(AI 함수·데이터 로더)를 함께 씁니다.
작성 내용 자동 저장은 서로 다른 키를 써서 섞이지 않습니다.

작성 도우미의 네 가지 도구:

1. **교수학습-평가 계획** — 성취기준을 고르면 단원명·성취기준이 공식 원문으로 채워지고,
   평가요소·수업방법·주안점은 AI로 생성 (성취기준 부분은 API 키 불필요)
2. **학습지로 주안점 만들기** — 학습지 파일에서 `[도입]`·`[수업]`·`[평가]` 생성
3. **학기단위 성취수준** — 계획에 담은 성취기준의 공식 성취수준을 이어붙임 (API 키 불필요)
4. **수행평가 영역별 세부 기준** — 평가기준·채점기준·총점 표

표는 화면 밖 사본(`.copy-source`)을 복사합니다. 편집칸의 `textarea` 값은 DOM 텍스트가
아니라 자바스크립트 속성이라, 편집용 표를 그대로 복사하면 칸이 비어서 붙기 때문입니다.
...
**중학교 고시 24개 과목 전체 + 승인 과목 1개 = 25과목 · 성취기준 723개 · 진술 3,343건**이
수록되어 있습니다.

| 5단계 (A~E) — 19과목 | 3단계 (A~C) — 6과목 |
| --- | --- |
| 과학 87 · 사회 74 · 수학 60 · 기술·가정 52 · 국어 51 · 역사 40 · 정보 25 · 도덕 22 · 영어 21 · 한문 12 · 데이터 분석과 인공지능 12 · 생활 외국어 8과목 132 | 체육 51 · 보건 26 · 환경 19 · 진로와 직업 14 · 미술 13 · 음악 13 |
...
학교자율시간에 편성하는 시도교육감 승인 과목은 고시 교육과정에 없어 대조할 기준값이
없습니다. 성취기준도 같은 문서에서 함께 읽어오고, 검증은 문서 내부 일관성으로 합니다
(수준 구성 통일 · 진술 잘림 · 성취기준 문장 완결성 · 코드 중복).

```bash
python scripts/extract-achievement-levels.py --approved \
  --source "출처 표기" "과목명" "경로/성취수준.pdf"
```

성취기준은 `data/curriculum-2022-approved.json` 에, 성취수준은 다른 과목과 같은 파일에
들어갑니다. 앱의 과목 목록에 `— 학교자율시간 승인 과목` 표시와 함께 나타납니다.
현재 수록: **데이터 분석과 인공지능** (12개 성취기준, 5단계).
